        ...
        {food.length === 0 ? <h1 className='mt-4 text-lg'>There is no food added in restauarnt</h1> : food.map((item, index) => {
          return (
            <div key={index} className='h-1/3 w-[30%] mt-4 mx-3 rounded-md shadow-lg shadow-gray-400' >
              <img src='./image.avif' alt={item.foodTitle} className='rounded-md'/>
              ...
              <div className='my-2 mx-2 flex flex-row'>
                <button onClick={() => {
                  setModalOpen(true);
                  setFoodItem(item);
                }} className='px-4 py-2 m-2 text-base font-medium bg-[#FEC013] hover:bg-[#ffd04f] rounded flex flex-row items-center'><FaEdit className='mr-2'/> Update</button>
                <button onClick={()=>handleDelete(item._id)} className='px-4 py-2 m-2 text-base font-medium bg-[#FEC013] hover:bg-[#ffd04f] rounded flex flex-row items-center'><IoTrashBin className='mr-2'/> Delete</button>
              </div>
            </div>
          )
        })}
